...
- **机器封面图**（`*_cover.png`——侧栏打印机图片与引导页）主程序**只从应用自带的 `resources/profiles/<厂商>/` 读取**（`Plater.cpp` / `WebGuideDialog.cpp` 均无数据目录回退），因此仅通过源仓库安装的厂商会显示占位图；需要主程序小改（加 `<数据目录>/vendor/<厂商>/` 回退），插件自身无法补齐。
