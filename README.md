...
- **Machine cover images** (`*_cover.png` — the printer picture in the sidebar and the guide page) are read **only** from the app's own `resources/profiles/<Vendor>/` today (`Plater.cpp` / `WebGuideDialog.cpp`, no data-dir fallback), so a vendor installed purely from a repo source shows a placeholder picture. This is a gap on the **main-app** side: the plugin stays standalone and only writes into the app's data directory. If imported vendors should show covers too, it belongs in the app itself as a first-class capability (data-dir installed vendors sharing the same asset resolution as bundled ones) — not as a plugin + main-app patch hybrid.
